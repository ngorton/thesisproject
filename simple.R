...
mypeople <- rep(NA, nrow(1000))

wage <- function(highwage, mortality.vector) {
  data = data.table(cost=rnorm(1000,100, 10), yc = 0, mortality.vector)
  data[yc==0, ':='(
    val_school =(mortality.vector)*(-1*cost*childhood)  + (1-mortality.vector)*highwage*(lifeexp-childhood),
    val_noschool =(mortality.vector)*(lowwage*childhood)  + (1-mortality.vector)*lowwage*(lifeexp-childhood)
  )]
...
expected.val <- function(mort) {
  no.school <- (1-mort)*(lifeexp*lowwage)
  school <- (1-mort)*(-childhood*cost + highwage*(lifeexp-childhood)) + mort*(-childhood*cost)
  return(no.school-school)
}
