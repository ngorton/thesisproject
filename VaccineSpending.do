...
estpost tabstat interpolated_school survival_rate vaxspending unicefmcv1 unicefmcv2 unicefdtp1 unicefdtp3 measles_prcnt_under5 dtp_prcnt_under5 lifeexpect  logpop loggni, by(gavi_status_00) statistics(mean sd max min) columns(statistics) listwise not 
esttab using tables.tex, replace main(mean) aux(sd) nostar unstack noobs nomtitle nonumber booktabs addnote("note")  eqlabels(`e(labels)') cells("mean(fmt(2)) sd(fmt(2)) min(fmt(2)) max(fmt(2))")  
